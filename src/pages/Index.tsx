    ...
    <div className="min-h-screen bg-background text-foreground py-4 md:py-6 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-300">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-6 md:mb-8">
          ...
          <p className="text-muted-foreground font-bold text-base md:text-lg">Stop overthinking, start doing.</p>
        </div>
        ...
        <Tabs defaultValue="edit" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-4 md:mb-6 bg-card border border-border rounded-xl md:rounded-2xl p-1 h-12 md:h-16">
          ...
          <TabsContent value="edit">
            <Card className="border-none shadow-2xl shadow-purple-500/10 rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden bg-card">
              <CardHeader className="border-b border-border py-4 md:py-6 px-4 md:px-8">
                <div className="flex justify-between items-center">
                  <CardTitle className="text-xl md:text-2xl font-black">Options</CardTitle>
                  <div className="flex gap-2 md:gap-3">
                    <Button variant="ghost" size="sm" onClick={handleSave} className="h-9 md:h-10 px-3 md:px-4 text-xs md:text-sm font-bold text-purple-600 dark:text-purple-400 hover:bg-purple-500/10 rounded-lg md:rounded-xl">
                      <Save className="w-3.5 h-3.5 md:w-4 md:h-4 mr-1 md:mr-1.5" /> Save
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => setOptions(['', ''])} className="h-9 md:h-10 px-3 md:px-4 text-xs md:text-sm font-bold rounded-lg md:rounded-xl">
                      <RotateCcw className="w-3.5 h-3.5 md:w-4 md:h-4 mr-1 md:mr-1.5" /> Reset
                    </Button>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="pt-4 md:pt-8 px-4 md:px-8 pb-6 md:pb-8">
                <div className="max-h-[40vh] md:max-h-[45vh] overflow-y-auto pr-1 md:pr-2 mb-4 md:mb-6 custom-scrollbar">
